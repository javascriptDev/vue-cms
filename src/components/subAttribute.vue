<template>
    <div style="padding-left:10px;">
        ...
        <div v-if="attr" v-for="(v,k,i) in attr">
            ...
            <mu-slider v-if="v.type==='slider'" v-model="v.value" :step="v.step" :max="v.max" :min="v.min" @mouseup.native="updateAttribute($event, k)" />
            <!--  标签型（label）属性  -->
            <div v-if="v.type==='label'" class="mu-text-field-label">{{v.value}}</div>
            <!--  开关（boolean）属性   -->
            <mu-switch v-if="v.type==='boolean'" :label="k" v-model="v.value" @change="updateAttribute($event, k)" labelLeft :style="{width:'100%',marginBottom:'10px'}" />
            <!--  选择型 (selection) 属性  -->
            <mu-select-field v-if="v.type==='selection'" v-model="v.value" :label="v.key || k" @input="updateAttribute($event, k)" style="width:100%;">
                <mu-menu-item v-for="(item,index) in v.items" :value="item" :title="item" :key="index" />
            </mu-select-field>
            <!-- 图标型 (icon) 属性 Muse-UI专用 -->
            <iconPicker v-if="v.type==='icon'" @change="updateAttribute($event, k)" v-model="v.value" :name="k"/>
            <!-- 图标型 (ionicon) 属性 iView-UI专用 -->
            <ioniconPicker v-if="v.type==='ionicon'" @change="updateAttribute($event, k)" v-model="v.value" :name="k"/>
            <!-- 颜色型 (color) 属性 -->
            <colorPicker v-if="v.type==='color'" @change="updateAttribute($event, k)" v-model="v.value" :name="k"/>
            <!-- 子属性 -->
