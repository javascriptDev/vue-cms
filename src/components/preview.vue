    ...
    methods: {
        ...
        getId(target) {
            ...
            function rec(dom){
                ...
                if(guidTest(dom.id)) {
                    console.log(`inner domID => ${dom.id}`)
                    id = dom.id
        ...
        clickPreview(e) {
            ...
            if (componentHTML) {
                ...
                //保存到vuex
                let currentId = this.getId(componentHTML)
        ...
        mount() {
            ...
            components.filter(component => !component.parentId).forEach(component => {
                mount(component.info.id, component, me.getAllChildSettings(component, components)).then(vm => {
