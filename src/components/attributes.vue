    ...
    methods: {
        updateAttribute(attr) { //属性值输入事件
            ...
            if (!components[index].parentId) {
                mount(component.info.id, component, this.getAllChildSettings(component, components))
            } else {
                //对于嵌套组件，更新其顶级父组件
                ...
                mount(topComponent.info.id, topComponent, this.getAllChildSettings(topComponent, components))
            }
...
/**
         * 获取所有子组件的data值,用于预览时渲染页面
         * @param  {string} parentId   父组件id
         * @param  {object} components 所有组件
         * @return {array}             一个组件下所有data
         */
        getAllChildSettings(parent, components) {
            let result = {}
